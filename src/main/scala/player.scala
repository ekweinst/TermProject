...
class Player(name: String, var health: Int, var armor: Int, var damage: Int, var level: Int, var experience: Int, var coins: Int):
	...
	def willItLevelUp: Unit = {
		if(getExperience >= ((4 * getLevel * getLevel)/5)) {
				setExperience(getExperience - ((4 * getLevel * getLevel) / 5))
				levelUp
			}
		else{
				println(getExperience)
			}
	}
  // Level up the player and update states
	def levelUp: Unit =
	    damage += Random.between(1,3)
	    health += Random.between(3,6)
	    armor += Random.between(1,2)
	    level += 1
	    println("You leveled up! you are now level: " + getLevel + " You now have: " + getDamage + " damage " + getHealth + " health and " +getArmor+" armor!")
